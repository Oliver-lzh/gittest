...
![image-20251021104311008](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219693.png)
...
![image-20251020214118568](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219705.png)

其次就是要使用erase删除键值对的原因是这样：

![image-20251020214308770](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219706.png)
...
![image-20251024160802241](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219707.png)
...
![image-20251028102632749](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219708.png)
...
![image-20251103143635125](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219709.png)
...
![image-20251103150840677](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219710.png)
...
这种题目的特征就是你需要先拉长滑动窗口的范围，之前靠移动左边缩短才能定位到真正的答案，这种就是叫做越短越合法。

因此答案收割都是在while循环收缩后。

**窗口越大越难满足，越小越容易满足”的性质，就叫“单调性”，正是滑动窗口适用的核心前提！**

![image-20251106100608226](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219711.png)
...
![image-20251029094920805](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219713.png)
...
**这次一开始没有全部通过是因为cnt没有初始化为0，导致错误发生。**
...
![image-20251104091346920](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219714.png)



![image-20251104091426891](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219715.png)
...
#### [904. 水果成篮 - 力扣（LeetCode）](https://leetcode.cn/problems/fruit-into-baskets/submissions/678211057/)
...
![image-20251107111315667](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219716.png)
...
#### [2904. 最短且字典序最小的美丽子字符串 - 力扣（LeetCode）](https://leetcode.cn/problems/shortest-and-lexicographically-smallest-beautiful-string/)#
...
public:
    string shortestBeautifulSubstring(string s, int k)
    {
        ...
        for(;right<s.size();right++)
        {
            ...
            while(num_one>k)
            {
                sub_result.push_back(s.substr(left,right));//截取长度错误
                if(s[left]=='1')
                {
                    num_one--;
                }
        ...
        int min_index=0;
        for(int i=0;i<sub_result.size();i++)//没有处理字典数最小的情况
        {
...
(1)在初始化 vector<string> sub_result的时候，因为没有提前分配空间，只能使用sub_result.push_back()的方式插入；

倘若想要使用sub_result[]插入，需要提前分配空间

```c++
vector<string> sub_result;
sub_result.resize(3); // 预分配 3 个空字符串

sub_result[0] = "apple"; // ✅ 现在可以用了
sub_result[1] = "banana";
sub_result[2] = "cherry";
```

(2)在截取一个长字符串的其中一个短的子字符串的时候，使用如下切割：
...
public:
    string shortestBeautifulSubstring(string s, int k)
    {
        ...
        for(int i=0;i<sub_result.size();i++)
        {
            ...
            else if(current_length == min_length)//比较字典数最小部分
            {
...
最美字符串：**包含恰好 k 个 '1' + 第一个字符是 '1'（无前导零，隐藏条件）**
...
public:
    int balancedString(string s)
    {
        ...
        for(;right<s.size();right++)
        {
            ...
            while(right-left+1>=4*k)
            {
                middle=abs(cnt['Q']-k)+abs(cnt['W']-k)+abs(cnt['E']-k)+abs(cnt['R']-k);
...
这里错误的地方在于，他需要的是替换的是一个子串，而不是我这里代码逻辑的一个一个的替换的个数。
...
#### [825. 适龄的朋友 - 力扣（LeetCode）](https://leetcode.cn/problems/friends-of-appropriate-ages/)#

这道题完全没有思路，完全是完整别人的思路抄写的。
...
![image-20250819113457615](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219717.png)

原因就是还是没有去除重复的字符才更新。如果是目标是“abc”,给出的字符串是abca,如果放在这个位置right=4,left=0,结果是4-0为4显然不对。

3.为什么这个接力结果有100多个不对？

![image-20250818220627628](https://gitee.com/oliver_go/image-hosting/raw/master/image/20251118104219718.png)
